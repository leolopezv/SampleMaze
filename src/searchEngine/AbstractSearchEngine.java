...
public class AbstractSearchEngine {
    ...
    public void reconstructPath(Dimension goal, HashMap<Dimension, Dimension> parentMap) {
        LinkedList<Dimension> path = new LinkedList<>();
        Dimension current = goal;

        while (current != null) {
            path.addFirst(current);  // Agregar cada nodo al camino
            current = parentMap.get(current);  // Moverse al nodo padre
        }

        // Almacenar el camino y actualizar la profundidad máxima
        this.searchPath = path.toArray(new Dimension[0]);
        this.maxDepth = path.size();
    }
}
